...
### extract read-stats
    ...
    Count the number of modifications per-read from an indexed modBAM file.
    Produces a CSV with one row per read, with columns for the read name
    and count for each modification code specified via --mod-codes.
    
    This subcommand is appropriate for quantification of modifications across
    reads (e.g. per-read m6A counts in direct-RNA data), when you do not need
    per-position genomic coordinates.
    
    Usage: modkit extract read-stats [OPTIONS] <IN_BAM> <OUT_CSV>
    ...
    Options:
          --mod-codes <MOD_CODES>
              Modification code(s) to count. Required argument. Specify the
              canonical base and the modification code, separated by a colon.
              For m6A use A:a. Multiple codes can be specified, e.g.
              --mod-codes A:a A:17596 T:17802.  See the SAM tags specification
              for details on modification codes.
    
          --reference <REFERENCE>
              Path to reference FASTA for extracting sequence context.
              (alias: ref)
          ...
          --no-header
              Suppress column header in the CSV output.
    ...
    Example (count m6A modifications per read from direct-RNA data):
      modkit extract read-stats input.bam output.csv --mod-codes A:a
    
    Example (count multiple modifications):
...
### Filtering extract output for specific modifications

`modkit extract` has three subcommands for producing per-read information:

- `extract read-stats` ? per-read modification counts (quantification across reads, use --mod-codes A:a for m6A)
- `extract full` ? probability-level base modification information (column 14 = mod_code)
- `extract calls` ? thresholded read-level modification calls (column 14 = call_code)

For per-read modification counts (recommended for m6A quantification in direct-RNA data):
```bash
# Count m6A (A:a) per read
modkit extract read-stats input.bam output.csv --mod-codes A:a

# Count multiple modification types at once
modkit extract read-stats input.bam output.csv --mod-codes A:a A:17596 T:17802
```

For base modification probability records or thresholded calls, use `extract full` or `extract calls` then post-process by column 14.

Both `extract full` and `extract calls` output all modifications in the BAM. To filter for a specific modification type (e.g. m6A only), post-process the output by filtering on column 14:

- In `extract full`, column 14 is **`mod_code`** (base modification code from the MM tag)
- In `extract calls`, column 14 is **`call_code`** (hard call of the modification)

The SAM modification code for m6A is `a`, for 5mC is `m`, for 5hmC is `h`, for inosine is `17596`, and for pseudouridine is `17802`.

Examples (filter for m6A only):

```bash
# Extract probability-level read information, filter for m6A
modkit extract full input.bam output.tsv
awk '$14 == "a"' output.tsv > output_m6A_only.tsv

# Extract thresholded read-level calls, filter for m6A
modkit extract calls input.bam calls.tsv
awk '$14 == "a"' calls.tsv > calls_m6A_only.tsv
```

Important notes:

- Use `--ref <reference.fa>` only when reference-context columns (chrom, ref_position) or motif-based filtering (`--motif`, `--cpg`) are needed. For simple modification-code filtering without reference context, `--ref` is optional.
- Options like `--motif <MOTIF> <OFFSET>` or `--cpg` restrict output to specific sequence motifs (e.g. CG dinucleotides). These are **not** a replacement for filtering by modification type.
- For site-level (per-position) modification counts with built-in modification filtering, use `modkit pileup --modified-bases m6A` instead of `modkit extract`.
- `--mod-codes A:a` is valid **only** with `modkit extract read-stats`. Do not use it with `extract full` or `extract calls`.
- Do **not** use `--modification m6A`, `--modified-bases m6A`, or `--mod-code a` with `extract full` or `extract calls` ? these are not supported.
- **Never** use bare `modkit extract input.bam output.tsv` without a subcommand ? it is not valid.
