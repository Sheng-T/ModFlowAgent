...
def _auto_pick_workflow_from_query(user_input: str):
    """Return a workflow spec when the request is explicit enough to avoid LLM ambiguity."""
    text = (user_input or "").lower()
    if not text:
        return None

    wants_command = any(k in text for k in [
        "generate",
        "command",
        "local",
        "本地",
    ...
    mentions_pacbio = any(k in text for k in [
        "pacbio",
        "hifi",
        ".bam",
    ])
    mentions_full_pipeline = any(k in text for k in [
        "methylong",
        "nextflow",
        "nf-core",
        "nfcore",
    ...
    if mentions_pacbio and wants_command and not mentions_full_pipeline:
        return wf_registry.get("pacbio_dna")

    if mentions_pacbio and mentions_full_pipeline:
        return wf_registry.get("methylong")
...
def retrieve_tool_docs_node(state: AgentState) -> AgentState:
    ...
    if is_workflow_request and workflow_type == "":
        # Workflow type not yet resolved — defer expensive per-workflow RAG
        # until a specific workflow has been selected.
        ui_print(f"\n[RAG] Workflow request (type not yet resolved) — deferring pipeline doc retrieval until selection.")
        state["rag_suggestion"] = {}
...
def plan_tool_steps_node(state: AgentState) -> AgentState:
    ...
    if is_workflow_request or workflow_type in ("nfcore", "local"):
        ...
        if selected_workflow and not state.get("workflow_candidates"):
            ...
            if state.pop("user_confirmed_workflow", False):
                ...
                if _spec:
                    ...
                    state["execution_profile"] = "workflow_nfcore" if _spec.type == "nfcore" else "workflow_local"
            ...
            if keyword_hit:
                ...
                if spec:
                    ...
                    state["execution_profile"] = "workflow_nfcore" if spec.type == "nfcore" else "workflow_local"
        ...
        heuristic_spec = _auto_pick_workflow_from_query(user_input)
        if heuristic_spec:
            _prev_wf = (state.get("local_prereq_params") or {}).get("_workflow", "")
            if heuristic_spec.name != _prev_wf:
                state["local_prereq_params"] = {}
                state["nfcore_prereq_params"] = {}
                ui_print(f"[Planner] Workflow changed -> cleared local/nfcore prereq params")
            state["selected_workflow"] = heuristic_spec.name
            state["workflow_type"] = heuristic_spec.type
            state["execution_profile"] = "workflow_nfcore" if heuristic_spec.type == "nfcore" else "workflow_local"
            state["tool_sequence"] = [heuristic_spec.name] if heuristic_spec.type == "nfcore" else list(heuristic_spec.steps)
            state["workflow_candidates"] = []
            if heuristic_spec.type == "local":
                _fetch_local_tool_docs(state, heuristic_spec)
            ui_print(f"[Planner] Heuristic-selected '{heuristic_spec.display_name}'")
            return state
    ...
    state["execution_profile"] = "single_tool"
    tools_args = "\n\n".join(
        format_tool_schema(TOOL_ARGS.get(tool_name, {}))
        for tool_name in identified_tools
    )
